...
@Component({
  selector: 'app-add-tracking-emp',
  templateUrl: './add-tracking-emp.component.html',
  encapsulation:ViewEncapsulation.None,
})
export class AddTrackingEmpComponent implements OnInit {
...
tracking_id_pk=0;
  ...
  AddTracking(form:NgForm)
  {
    ...
    this.service.AddTracking(this.trackingData).subscribe(res=>{
      ...
      {
        this.resetForm(form);
        this.router.navigate(['/employee/show-tracking-emp'],{relativeTo:this.route});
      }
      else{
        console.log(res);
      }
